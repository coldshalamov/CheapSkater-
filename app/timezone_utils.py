...
def convert_utc_to_regional_time(
    dt: datetime | str | None, region: str | None
) -> datetime | None:
    ...
    if isinstance(dt, str):
        try:
            # Handle 'Z' suffix if present (common in JS/ISO strings)
            if dt.endswith("Z"):
                dt = dt[:-1] + "+00:00"
...
def format_regional_timestamp(
    dt: datetime | str | None, region: str | None, format_str: str = "%I:%M %p"
) -> str:
...
def format_regional_full_timestamp(
    dt: datetime | str | None, region: str | None
) -> str:
