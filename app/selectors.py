...
PRICE = ":scope :is([data-test*='price'], [data-automation-id*='price'], [data-testid*='price'], .price, .sale-price, [class*='Price'], [aria-label^='$'])"
PRICE_ALT = ":scope [data-testid*='price'], :scope [aria-label^='$']"
WAS_PRICE = ":scope :is([data-test*='was'], .was-price, .strike, [aria-label*='Was'])"
AVAIL = ":scope :is([data-test*='availability'], [data-automation-id*='availability'], [data-testid*='availability'], .availability, [aria-label*='stock'])"
IMG = ":scope img"
LINK = f":scope a[href*='{PRODUCT_PATH_FRAGMENT}']"
NEXT_BTN = "nav[aria-label='Pagination'] a[rel='next'], button[aria-label='Next'], .pagination-next a, .pagination button[aria-label='Next'], [rel='next']"
STORE_BADGE = "header :is([aria-label*='My Store'], [aria-label*='store'], [data-test*='store'], [data-automation-id*='store'], a[href*='store-details'])"
